...
#ile = open(BOARDS[i - 1], 'r')

NB = 5 #len(file.readlines())  # Board number of rows/columns
...
class State:
    ...
    def valid_move(self, move):
        ...
        if not (inside(move.xi, move.yi,self.nb) and inside(move.xf, move.yf,self.nb)):
            print("A")
            return False
        ...
        for i in range(len(self.matrix)):
            for j in range(len(self.matrix[0])):
                if self.matrix[i][j] == player:
                    for dx in range(-2, 3):
                        for dy in range(-2, 3):
                            if self.nb > i + dx >= 0 and self.nb > j + dy >= 0:
    ...
    def multiply(self, move):
        ...
        for dx in range(-1, 2):
            for dy in range(-1, 2):
                if inside(move.xf + dx, move.yf + dy,self.nb):
    ...
    def isEndState(self):
        ...
        return self.available_moves(1).size == 0 or self.available_moves(-1).size == 0
...
'''class Attax:
    ...
    def is_winner(self):
        ...
        if self.valid_plays(self.board.player) > 0:
            return -2
        elif self.board.count_pieces(1) > self.board.count_pieces(-1):
            return 1
        elif self.board.count_pieces(1) < self.board.count_pieces(-1):
            return -1
